...
class Mask(object):
    def __init__(self, w, h, resize,sub_w_num, sub_h_num, dense_rate=0):
        self.w = w
        self.h = h
        self.sub_w_num = sub_w_num
        self.sub_h_num = sub_h_num
        self.target_num_max = sub_w_num*sub_h_num
        self.target_num_range = [1, 16]
    ...
    def single_mask(self, target_num=None):
        ...
        cell_w = self.w // self.sub_w_num
        cell_h = self.h // self.sub_h_num
        for i in range(self.target_num):
            w_id, h_id = divmod(pos_id_list[i], self.sub_w_num)
            x_pos = random.randint(0, cell_w - 1) + w_id * cell_w
            y_pos = random.randint(0, cell_h - 1) + h_id * cell_h
